...
class BlurDetection:
    ...
    def _blurDetection(self, imgName):
        if isinstance(imgName,str):
            ...
            img2gray = self.processImg(imgName)
        #imgMat=self._imageToMatrix(img2gray)/255.0
        img2gray = img2gray/255.0
        x, y = img2gray.shape
        score = [img2gray[idx+2,:]-img2gray[idx,:] for idx in range(x-2)]
    ...
    def _Tenengrad(self,imgName):
        ...
        if isinstance(imgName,str):
            ...
            img2gray = self.processImg(imgName)
        #f = self._imageToMatrix(img2gray)/255.0
        source = filters.sobel(img2gray)
        source=np.sum(source**2)
    ...
    def _drawImgFonts(self, img, strContent):
        ...
        cv2.putText(img, strContent, (0, 20), font, fontSize, (0, 255, 0), 1)
...
if __name__ == "__main__":
    BlurDetection = BlurDetection(strDir="/data/DataSets/yunli_test_db/00001") #recog_match/00007
    #BlurDetection.Test_Tenengrad () # TestSMD T=8.0
    BlurDetection.TestBrener() # T=60
