...
import argparse
import numpy as np
sys.path.append(os.path.join(os.path.dirname(__file__),'../obj_detector'))
        ...
        while self.vdo.grab(): 
            ...
            if cnt % 5 ==0 or detect_fg:
                ...
                if bbox_xcycwh is not None:
                    ...
                    #detect_xyah = box_xyah
                    self.track_init(box_xcycah)
                    detect_fg = False
                    #show_box = np.hstack([bbox_xcycwh[:,:2]-bbox_xcycwh[:,2:]/2,bbox_xcycwh[:,:2]+bbox_xcycwh[:,2:]/2])
                    self.track_predict()
            ...
            if bbox_xcycwh is not None:
                ...
                outputs = self.deepsort.update(bbox_xcycwh, cls_conf, im,update_fg)
            end = time.time()
            consume = end-start
            if len(outputs) > 0:
                bbox_xyxy = outputs[:,:4]
...
def parse_args():
    ...
    parser.add_argument("--yolo_cfg", type=str, default="../obj_detector/cfg/yolo_v3.cfg")
    parser.add_argument("--yolo_weights", type=str, default="../../models/yolov3.weights")
    parser.add_argument("--yolo_names", type=str, default="../obj_detector/cfg/coco.names")
