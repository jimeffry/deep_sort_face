...
class MtcnnDetector(object):
    ...
    """
    def __init__(self,threshold,cropsize,model_folder='../../../models/mx_model/', \
                factor=0.709,accurate_landmark=False):
        """
            Parameters:
                model_folder : string
                    path for the models
        ...
        self.minsize   = float(config.min_size)
    ...
    def get_topk(self,bboxes):
        '''
        bboxes: [x1,y1,x2,y2,score,reg]
        return: according to score ,return topk
        '''
        score = bboxes[:,4]
        keep_idx = score.argsort()[::-1]
        if len(keep_idx) > config.topk:
            keep_idx = keep_idx[:config.topk]
    ...
    def detectFace(self, img,db_used=False):
        """
        ...
        """
        assert len(img.shape)==4 ,"Please input image:[b,w,h,c]"
        img_crops_list = []
        #img = img_input[0,...]
        # check input
        #if img is None:
         #   return img_crops_list
        # only works for color image
        if len(img.shape) != 4:
            return img_crops_list
        #mt_dface = TimeR('net.face_detect')
        t = time.time()
        #first stage
        #mt_pnet = TimeR('net.face_detect.pnet')
        batch_rectangles = self.Pnet_Process(img)
        #mt_pnet.finish()
        t1 = time.time() - t
        # second stage
        t2=0
        ...
        for idx in range(img.shape[0]):
            #mt_rnet = TimeR('net.face_detect.rnet')
            if len(batch_rectangles[idx])<1:
                img_crops_list.append([])
                continue 
            t = time.time()
            rectangles = self.Rnet_Process(batch_rectangles[idx],img[idx,...])
            #mt_rnet.finish()
            if len(rectangles)==0:
                img_crops_list.append([])
                continue
            ...
            t = time.time()
            # third stage
            #mt_onet = TimeR('net.face_detect.onet')
            rectangles = self.Onet_Process(rectangles,img[idx,...])
            #mt_onet.finish()
            t3 += (time.time() - t)
            if len(rectangles)==0:
                img_crops_list.append([])
                continue
            if db_used:
                img_crops_list.append(rectangles)
                continue
            points = np.reshape(rectangles,[-1,15])
            points = points[:,5:]
            points_list = points.tolist()
            img_crops = alignImg(img[idx,...],self.cropsize,points_list)
            if config.img_show:
                label_show(img[idx,...],rectangles)
            if len(img_crops) > 0:
                img_crops_list.append(img_crops)
            else:
                img_crops_list.append([])
        if config.time:
