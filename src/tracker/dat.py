...
class DAT(BaseCF):
    ...
    def init(self,first_frame,bbox):
        ...
        #prob_lut_ is color hist ****** p(o|O,s) = p(o|O)/[p(o|O)+p(o|s)]
    ...
    def update(self,current_frame,vis=False):
        ...
        search_sz_w=int(np.floor(target_sz[0]+self.config.search_win_padding*target_sz[0])) #max(target_sz[0],target_sz[1])))
        search_sz_h=int(np.floor(target_sz[1]+self.config.search_win_padding*target_sz[1])) #max(target_sz[0],target_sz[1])))
        ...
        if self.config.distractor_aware is True:
            ...
            pm_search=(pm_search+pm_search_dist)/2  #**** p(o)= lamd*p(o|s)+lamd* p(o|d)
        # plt.subplot(122)
...
def get_foreground_background_probs(frame,obj_rect,num_bins,bin_mapping=None):
    ...
    # test_hist = cv2.calcHist([frame],[0],None,[16],[0,256])
    # test_obj = cv2.calcHist([obj_win],[0],None,[16],[0,256])
    # test_prob = (test_obj+1)/(test_hist+2)
    # plt.subplot(131)
    # print(np.shape(test_hist))
    # plt.plot(test_hist)
    # plt.subplot(132)
    # plt.plot(test_obj)
    # plt.subplot(133)
    ...
    # plt.show()
    # cv2.waitKey(0)
    #print('Ho and Hs size',np.shape(obj_hist),np.shape(surr_hist),np.shape(prob_lut))
    prob_map = None
...
def get_foreground_prob(frame,prob_lut,bin_mapping):
    frame_bin=cv2.LUT(frame,bin_mapping).astype(np.int64)
    prob_map = prob_lut[frame_bin[:, :, 0], frame_bin[:, :, 1], frame_bin[:, :, 2]]
    #print('foreground prob',np.shape(frame_bin),np.shape(prob_map))
    # plt.subplot(111)
    # plt.imshow(prob_map)
    # plt.colorbar()
    # plt.show()
...
def get_nms_rects(prob_map,obj_sz,scale,overlap,score_frac,dist_map,include_inner):
    ...
    #print('search and obj',(width,height),rect_sz)
    #print('len posx posy',len(posx),len(posy))
    ...
    #print('integral map',np.shape(prob_map),np.shape(int_prob_map))
    ...
    #print("print propesal obj:",len(v_scores))
    top_rects = []
    top_vote_scores = []
    top_dist_scores = []
